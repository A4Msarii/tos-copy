...
if (isset($_REQUEST['subId'])) {
    ...
    $fetchSubData = $connect->query("SELECT * FROM subcheclist WHERE id = '$subId'");
...
if (isset($_REQUEST['editSub'])) {
    ...
    $updateQuery = "UPDATE subcheclist SET name = '$subChecklist_name', description = '$description', status = '$status', priority = '$priority', category = '$category', comment = '$comment' WHERE id = '$subCheckId'";
    $statement_editor = $connect->prepare($updateQuery);
    $statement_editor->execute();

    $_SESSION['success'] = "Subcheck List Updated Successfully";
    header("Location:subCheckList.php?checkListId=" . $mainCheck);
}
...
if ($_REQUEST['deleteId']) {
    ...
    $sql = "DELETE FROM subcheclist WHERE id = '$deleteId'";
