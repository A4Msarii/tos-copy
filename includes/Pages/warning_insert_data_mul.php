...
foreach ($cat as $key =>$cats) 
{
    ...
    if($cats=="test"){
           $sql = "INSERT INTO warning_category_data (warning,category,category_phase,category_value,grade,count,threshold,group_id)
            VALUES ('$warning_id','$cats','$phase_selesteds','$cat_datas1','$test_mark','$count','$value_mark','$group_id')";
     ...
     $sql = "INSERT INTO warning_category_data (warning,category,category_phase,category_value,grade,count,threshold,group_id)
     VALUES ('$warning_id','$cats','$phase_selesteds','$cat_datas','$grades','$count','$thri','$group_id')";
    $statement = $connect->prepare($sql);
    $statement->execute();
   }
}
