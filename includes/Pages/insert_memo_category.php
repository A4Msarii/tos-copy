...
if (isset($_POST["savememo"])) {
	if (empty($_POST["category"])) {
		...
		foreach ($category as $key => $value) {
			...
			$sym_mark = $marks[$key];
			$que_phase = "SELECT category FROM memocate where category='$sym_cate'";
			$stat_phase = $connect->prepare($que_phase);
			$stat_phase->execute();

			if ($stat_phase->rowCount() == 0) {
				$query_phase = "INSERT INTO memocate (category,marks,date) VALUES ('$sym_cate','$sym_mark',NOW())";
