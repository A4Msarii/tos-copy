...
if (isset($_REQUEST['addNewFile'])) {
    ...
    $mainCheckFileId = $_REQUEST['subCheckFileId'];
    ...
    if ($dupliData > 0) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $uploadPath)) {
            ...
            $query = "INSERT INTO personalsubchecklist_files (fileId,subCheckId) VALUES ('$lastId','$mainCheckFileId')";
            $stmt = $connect->prepare($query);
            $stmt->execute();
...
if (isset($_REQUEST['addSelectFile'])) {
    ...
    while ($name < count($fileId)) {
        $fId = $fileId[$name];
        $query = "INSERT INTO personalsubchecklist_files (fileId,subCheckId) VALUES ('$fId','$mainCheckFileId')";
        $stmt = $connect->prepare($query);
        $stmt->execute();
        $name++;
    }
