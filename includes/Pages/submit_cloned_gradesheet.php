...
$grades1=$_REQUEST['grades'];
                        //extra item subitem gradeing
                    foreach($item_types as $ie=>$index){
                        $item_get=$item_ids[$ie];
                        $grades=$index.$item_get;
                        $finals=$grades1[$grades];
                        $sql15 = "SELECT count(*) FROM `extra_item_subitem_grades_cl` WHERE user_id='$stud_db_id' AND item_id='$item_get' and type='$index' and cloneid='$get_clone_ides'";
                        ...
                        $sql12 = "UPDATE `extra_item_subitem_grades_cl`
                                        SET  `grade`='$finals'
                                        WHERE user_id='$stud_db_id' AND item_id='$item_get' and type='$index' and cloneid='$get_clone_ides'";
                        
                        $statement12 = $connect->prepare($sql12);
                        $statement12->execute();
                    } else {
                        $sql_b1 = "INSERT INTO extra_item_subitem_grades_cl (user_id,item_id,grade,date,type,cloneid) VALUES ('$stud_db_id','$item_get','$finals',NOW(),'$index','$get_clone_ides')";
...
if ($st5->rowCount() > 0) {
    ...
    foreach ($re5 as $row5) {
        ...
        $q2 = "SELECT * FROM warning_category_data where grade='$overall_grade' and warning='$warning_value'";
        ...
        if ($st2->rowCount() > 0) {
            ...
            foreach ($re2 as $row2) {
                ...
                $table = $row2['category'];
                $sql4 = "SELECT count(`id`) as check_exist FROM notifications WHERE user_id='$stud_db_id' and data='$set_id' and extra_data='threshold'";
                $res4 = $connect->query($sql4);
                $get_count4 = $res4->fetchColumn();
                $sql5 = "SELECT count(`id`) as check_exist_count FROM notifications WHERE user_id='$stud_db_id' and data='$set_id' and extra_data='reached_cout'";
                ...
                if ($table == "actual" && $class_name=="actual") {
                    ...
                    if ($table_value != "all") {
                        ...
                        } else if ($get_count10 >= $countss && $countss != 0) {
                            if ($get_count5 == 0) {
                                $msg = "reached_cout";
                                send_noti($stud_db_id, $set_id, $instructor_id, $msg,$cat_id);
                            }
                ...
                if ($table == "sim" && $class_name=="sim") {
                    if ($table_value == "all") {
                        ...
                        } else if ($get_count11 >= $countss && $countss != 0) {
                            if ($get_count5 == 0) {
                                $msg = "reached_cout";
                                send_noti($stud_db_id, $set_id, $instructor_id, $msg,$cat_id);
                            }
...
$update_read="UPDATE `notifications` SET `is_read` = 1 WHERE `user_id`='$stud_db_id' and `type`='$class_name' and `data`='$class_id' and extra_data='cloned_gradsheet' and permission='$get_clone_ides'";
...
{
   $notification = "INSERT INTO notifications (user_id,to_userid,type,data,extra_data,date) VALUES ('$instructor_id','$stud_db_id','$class_name','$class_id','filled your repeated gradsheet for',CURRENT_TIMESTAMP)";
...
foreach($itemd as $items){
    $sql522 = "SELECT count(*) FROM `extra_item_subitem_cl` WHERE item_id='$items' AND data_type='item'";
    $result522 = $connect->prepare($sql522);
    $result522->execute();
    $number_of_rows122 = $result522->fetchColumn();
    if($number_of_rows122==0){
    $sql_hash = "INSERT INTO  extra_item_subitem_cl (item_id,user_id,class_id,class,data_type,cloneid) VALUES ('$items','$stud_db_id','$class_id','$class_name','item','$get_clone_ides')";
    $statement_hash = $connect->prepare($sql_hash);
    $statement_hash->execute();
    }
 }
 foreach($subitemd as $subitems){
    $sql521 = "SELECT count(*) FROM `extra_item_subitem_cl` WHERE item_id='$subitems' AND data_type='subitem'";
    $result521 = $connect->prepare($sql521);
    $result521->execute();
    $number_of_rows121 = $result521->fetchColumn();
    if($number_of_rows121==0){
    $sql_hash1 = "INSERT INTO  extra_item_subitem_cl (item_id,user_id,class_id,class,data_type,cloneid) VALUES ('$subitems','$stud_db_id','$class_id','$class_name','subitem','$get_clone_ides')";
    $statement_hash1 = $connect->prepare($sql_hash1);
    $statement_hash1->execute();
 }
}
             ...
             if($subjects == "item"){
              ...
              $statement = $connect->prepare($sql);
               $statement->execute();
                 ...
                 $sql_a = "INSERT INTO item_clone_gradesheet (user_id,item_id,grade,cloned_id,date) VALUES ('$stud_db_id','$item_ids','$final_grade','$get_clone_ides',NOW())";
                 // echo $sql_a;
