        ...
        <div class="col-6">
          ...
          <div class="d-flex align-items-center">
            ...
            ?>
              ...
              <input type="hidden" value="<?php echo  $CourseCode11 ?>" id="get_name_courseCode">
              <script>
                $(document).ready(function() {
                  const cCode = $("#set_values1s11").val();
                  const cCourseCode = $("#get_name_courseCode").val();
                  localStorage.setItem('courseId', cCourseCode);
                  localStorage.setItem('courseClass', cCode);
                  // alert(cCourseCode);
            ...
            <div class="flex-grow-2 ms-5" style="text-align: center;">
              ...
              <span class="card-text text-body text-dark" style="font-weight:bold;"><?php echo $class; ?></span><br>
              <?php
              $fetchFolId = $connect->query("SELECT id FROM folders WHERE phaseId = '$phaseIdData'");
              $folId = $fetchFolId->fetchColumn();
              $fetchPhaseName = $connect->query("SELECT phasename FROM phase WHERE id = '$phaseIdData'");
              $phaseNameId = $fetchPhaseName->fetchColumn();
              $fetchShopId = $connect->query("SELECT shopid FROM user_briefcase WHERE folderId = '$folId' LIMIT 1");
              $shopId = $fetchShopId->fetchColumn();
              ?>
            <a style="background-color: #7901c1;color: white;border: 1px solid #7901c1;font-size: large; font-weight:bold;" target="_blank" href="<?php echo BASE_URL; ?>includes/Pages/fheader1.php?folder_ID=<?php echo $folId; ?>&shopId=<?php echo $shopId; ?>" class="btn btn-success" title="Open <?php echo $phaseNameId;?> Folder">
              <img src="<?php echo BASE_URL;?>assets/svg/new/ll_white.png" style="height:25px; width: 15px; margin: 3px;">
              <?php echo $phaseNameId;?></a>
            </div>
          </div>
...
<?php
if (isset($_REQUEST['noti_id'])) {
  $divName = $connect->query("SELECT divisionName FROM division WHERE id = '$dId'");
  $divData = $divName->fetchColumn();

  $divcolor = $connect->query("SELECT color FROM division WHERE id = '$dId'");
  $divColor = $divcolor->fetchColumn();
?>

  <script>
    $(document).ready(function() {
      $("#diviId").html("<?php echo $divData; ?>");
      $("#diviId").css("background-color", "<?php echo $divColor; ?>");
    });
  </script>
<?php
}
