...
$ide=$_REQUEST['cl_data'];
    //clear cleanrance
                foreach($ide as $id){
                    $query_ac ="INSERT INTO clearance_student_id (clearance_id,stud_id,class_id,class_table,clone_cid) VALUES ('$id','$stud_db_id','$class_id','$class_name',NULL)";
...
//additional adding
        ...
        //#################################3
         ...
         foreach($add_clear as $ides){
           echo $query_ad="UPDATE `additional_task`
            ...
            $sql15 = "SELECT count(*) FROM `extra_item_subitem_grades` WHERE user_id='$stud_db_id' AND item_id='$item_get' and `type`='$index1'";
            ...
            if ($number_of_rows11 > 0) {
                $sql12 = "UPDATE `extra_item_subitem_grades`
                            SET  `grade`='$finals'
                            WHERE user_id='$stud_db_id' AND item_id='$item_get' and `type`='$index1'";
                                $statement12 = $connect->prepare($sql12);
                                $statement12->execute();
            } else {
                $sql_b1 = "INSERT INTO extra_item_subitem_grades (user_id,item_id,grade,`date`,`type`) VALUES ('$stud_db_id','$item_get','$finals',NOW(),'$index1')";
...
if ($get_datast->rowCount() > 0) {
    ...
    // $error = "<div class='alert alert-success'>Gradsheet updated Successfully..</div>";
                $_SESSION['success'] = "Gradsheet Updated Successfully..";
