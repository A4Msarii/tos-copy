...
if (isset($_REQUEST['attachement'])) {
    ...
    for ($i = 0; $i < $totalfiles; $i++) {
        ...
        $dupliQuery = $connect->query("SELECT count(*) FROM per_subchecklistfile WHERE filename = '$filename'");
        ...
        if ($dupliData > 0) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"][$i], $uploadPath)) {
                $query = "INSERT INTO per_subchecklistfile (filename,fileType,checkId,subCheckId) VALUES ('$filename','$ext','$checkId','$subCheckId')";
...
if (isset($_REQUEST['driveLink'])) {
    ...
    while ($name < count($countPhase)) {
        ...
        $dupliQuery = $connect->query("SELECT count(*) FROM per_subchecklistfile WHERE lastName = '$location'");
        $dupliData = $dupliQuery->fetchColumn();
        // echo $dupliData;
        // die();
        if ($dupliData > 0) {
            ...
            $sql = "INSERT INTO per_subchecklistfile (fileName,lastName,fileType,checkId,subCheckId) VALUES ('$str','$location','$type','$checkId','$subCheckId')";
