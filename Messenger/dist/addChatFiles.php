...
if (isset($_REQUEST['attachement'])) {
    ...
    for ($i = 0; $i < $totalfiles; $i++) {
        ...
        $dupliQuery = $connect->query("SELECT count(*) FROM groupchats WHERE messages = '$filename'");
        ...
        if ($dupliData > 0) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"][$i], $uploadPath)) {
                ...
                $query = "INSERT INTO groupchats (senderId,groupId,messages,lastName,date,loca) VALUES ('$userId','$senderId','$lastId','file',NOW(),'userfile')";
                $stmt = $connect->prepare($query);
                $stmt->execute();

                // $query = "INSERT INTO groupchats (senderId,groupId,messages,lastName,fileType,date) VALUES ('$userId','$senderId','$filename','file','$ext',NOW())";
                // $stmt = $connect->prepare($query);
                // $stmt->execute();
...
if (isset($_REQUEST['driveLink'])) {
    ...
    while ($name < count($countPhase)) {
        ...
        $dupliQuery = $connect->query("SELECT count(*) FROM groupchats WHERE messages = '$location'");
        $dupliData = $dupliQuery->fetchColumn();
        // echo $dupliData;
        // die();
        $fileLoc = "chat";
        ...
        if ($dupliData > 0) {
            ...
            $sql = "INSERT INTO groupchats (senderId,groupId,messages,date,loca) VALUES ('$userId','$senderId','$lastId',NOW(),'userfile')";
            $stmt = $connect->prepare($sql);
            $stmt->execute();
...
if (isset($_REQUEST['onlineLink'])) {
    ...
    while ($name < count($countPhase)) {
        ...
        $dupliQuery = $connect->query("SELECT count(*) FROM groupchats WHERE lastName = '$location'");
        $dupliData = $dupliQuery->fetchColumn();
        // echo $dupliData;
        // die();
        $fileLoc = "chat";
