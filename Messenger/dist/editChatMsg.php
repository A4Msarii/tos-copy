...
if (isset($_REQUEST['editChatMsgBtn'])) {
    ...
    $updateQuery = "UPDATE chats SET messages = '$message',msgEdit = 'Edited' WHERE id = '$msgId'";
...
if (isset($_REQUEST['deleteForme'])) {
    ...
    $query1 = "INSERT INTO chatdeleteforme(msgId,userId) VALUES ('$deleteForMe','$userId')";
...
if (isset($_REQUEST['deleteForevery'])) {
    ...
    $sql = "DELETE FROM chats WHERE id = '$deleteForMe'";
...
if (isset($_REQUEST['replyBtn'])) {
    ...
    $query1 = "INSERT INTO chats(senderId,receiverId,messages,date,replyMsg,replyStatus) VALUES ('$userId','$chatId','$replyMessage',NOW(),'$reply','yes')";
    $statement1 = $connect->prepare($query1);
    $statement1->execute();
    header("Location:chat-direct.php?chatId=" . $chatId . "");
}
...
if (isset($_REQUEST['forwardBtn'])) {
    ...
    while ($name < count($memberId)) {
        ...
        $query1 = "INSERT INTO chats(senderId,receiverId,messages,date) VALUES ('$userId','$memberId1','$forwardMessage',NOW())";
        $statement1 = $connect->prepare($query1);
        $statement1->execute();
        $name++;
    }
...
if (isset($_REQUEST['forwardDocBtn'])) {
    ...
    while ($name < count($memberId)) {
        $memberId1 = $memberId[$name];
        $sql = "INSERT INTO chats (senderId,receiverId,messages,lastName,date,loca) VALUES ('$userId','$memberId1','$forwardDocId','file',NOW(),'userfile')";
...
if (isset($_REQUEST['editLinkBtn'])) {
    ...
    $updateQuery = "UPDATE chats SET msgEdit = 'Edited' WHERE id = '$mainId'";
    $statement_editor = $connect->prepare($updateQuery);
    $statement_editor->execute();
    header("Location:chat-direct.php?chatId=" . $chatId . "");
}
...
if (isset($_REQUEST['forwardLinkBtn'])) {
    ...
    while ($name < count($memberId)) {
        $memberId1 = $memberId[$name];
        $sql = "INSERT INTO chats (senderId,receiverId,messages,date) VALUES ('$userId','$memberId1','$linkId',NOW())";
        $stmt = $connect->prepare($sql);
        $stmt->execute();
        $name++;
    }
...
if (isset($_REQUEST['forwardEditorBtn'])) {
    ...
    while ($name < count($memberId)) {
        $memberId1 = $memberId[$name];
        $query1 = "INSERT INTO chats(senderId,receiverId,messages,date,loca) VALUES ('$userId','$memberId1','$forwardEditorId',NOW(),'page')";
        $statement1 = $connect->prepare($query1);
        $statement1->execute();
        $name++;
    }
